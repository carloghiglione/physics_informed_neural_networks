...
# path_to_tables_Leo = "/Users/leopere/napde_project_bgp/Solvers/Comet_solver_mu_theta/Tables"
# path_to_tables = path_to_tables_Leo
...
#I randomly choose the indexes of geometrical points to keep
train_split = 0.85 # % of data to keep in train set
n_choice_int = round(train_split*n_int_per_param)
n_choice_bc = round(train_split*n_bc_per_param)
indexes_int = np.array(range(0,int(n_int_per_param)))
indexes_bc = np.array(range(0,int(n_bc_per_param)))
np.random.shuffle(indexes_int)
np.random.shuffle(indexes_bc)
indexes_int_train = np.sort(indexes_int[range(n_choice_int)])
indexes_bc_train = np.sort(indexes_bc[range(n_choice_bc)])
indexes_int_test = np.sort(np.delete(indexes_int,range(n_choice_int)))
indexes_bc_test = np.sort(np.delete(indexes_bc,range(n_choice_bc)))
    ...
    if i in idx_train_mu:
        for j in range(n_theta):
            if j in idx_train_theta:
                train_int = train_int.append( data_int.iloc[ i*n_theta*n_int_per_param + j*n_int_per_param + indexes_int_train ] )
                train_bc = train_bc.append( data_bc.iloc[ i*n_theta*n_bc_per_param + j*n_bc_per_param + indexes_bc_train ] )
                test_int = test_int.append( data_int.iloc[ i*n_theta*n_int_per_param + j*n_int_per_param + indexes_int_test ] )
                test_bc = test_bc.append( data_bc.iloc[  i*n_theta*n_bc_per_param + j*n_bc_per_param + indexes_bc_test ] )
            else:
                test_int = test_int.append( data_int.iloc[ i*n_theta*n_int_per_param + j*n_int_per_param + indexes_int_test ] )
                test_bc = test_bc.append( data_bc.iloc[ i*n_theta*n_bc_per_param + j*n_bc_per_param + indexes_bc_test ] )
    else:
...
x_p_final    = tf.constant(np.array(test_int.reset_index().loc[range(int(n_int_per_param))][['x','y','mu','theta']]), ns.config.get_dtype())
x_final    = tf.constant(np.array(test_int.reset_index().loc[range(int(n_int_per_param))][['x','y']]), ns.config.get_dtype())
u_final      = tf.constant(np.array(test_int.reset_index().loc[range(int(n_int_per_param))][['u']]),                  ns.config.get_dtype())
u_final_norm = ( u_final - min_train )/( max_train - min_train )
...
ax.scatter(x_final[:,0], x_final[:,1], u_final_norm, label = 'exact solution')
ax.scatter(x_final[:,0], x_final[:,1], model(x_p_final).numpy(), label = 'num sol')
ax.legend()