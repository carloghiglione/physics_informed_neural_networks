...
param_geo_dim = 4
param_cond = 1
input_dim = param_geo_dim + param_cond + 3
...
geo_params = pd.read_table(os.path.join(cwd, 'p_geo_tab.txt'), sep=' ', header=None, names = ('geo1','geo2','geo3','geo4'))
geo_params = geo_params['geo1'].to_numpy()
n_params = len(geo_params)
n_nodes = int(data.shape[0]/n_params)


#load conducibility parameters and insert them in the table
p_cond = np.reshape(pd.read_table(os.path.join(cwd, 'p_cond.txt'), header=None).to_numpy(), (n_params,))
data['cond'] = 1
ident_all = np.ones((data.shape[0],))
for i in range(n_params):
    idx = range(n_nodes*i, n_nodes*i+n_nodes)
    data.loc[idx,'cond'] = p_cond[i]
    ident_all[idx] = i

ident_all = ident_all.astype(int)
# %% Undersample the data
n_under = 1000                 #number of nodes used for each mesh
data_under = pd.DataFrame()
AT_under = np.array([])

# I create a support vector to uniquely identify the block of soluzions inside 
ident = np.array([])

for i in range(n_params):
    idx_under = np.random.choice(range(n_nodes), n_under, replace = False)
    data_under = data_under.append( data[ident_all == i].iloc[idx_under] )
    AT_under = np.append(AT_under, AT_norm[n_nodes*i + idx_under])
    ident = np.append(ident, i*np.ones((n_under,)))
...
for i in range(n_params):
    ...
    test_2 = test_2.append( data[ident_all == i].iloc[idx] )
...
ns.minimize(pb, 'keras', tf.keras.optimizers.Adam(learning_rate=1e-2), num_epochs = 2500)
...
# idx_sort_real = np.argsort(geo_params)
# idx_sort_train = np.argsort(AT_max_train)
# idx_sort_test_1 = np.argsort(AT_max_test_1)
# idx_sort_test_2 = np.argsort(AT_max_test_2)

# fig2 = plt.figure()
# plt.plot(geo_params[idx_sort_real], 0.1*AT_max_real[idx_sort_real], 'k--s', markersize=5, label="experimental")
